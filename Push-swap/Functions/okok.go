...
func PushSwap1(stackA, stackB *[]int, instructions *[]string) {
	...
	if len(*stackA) > 3 {
		if sort.IntsAreSorted((*stackA)[1:]) && (*stackA)[0] > (*stackA)[1] && (*stackA)[0] < (*stackA)[len(*stackA)-1] {
			*instructions = append(*instructions, "sa")
			...
			PushSwap1(stackA, stackB, instructions)
			...
			*stackA = append((*stackA)[len(*stackA)-1:], (*stackA)[:len(*stackA)-1]...)
			...
			count, pos := minNumber1(stackA)
			moveToTop1(stackA, instructions, count, pos)
			if !sort.IntsAreSorted(*stackA) {
		...
		*instructions = append(*instructions, "sa")
		...
		smallStack1(stackA, instructions)
	}
...
func minNumber1(stackA *[]int) (int, int) {
	min := (*stackA)[0]
	count := 0
...
func smallStack1(stackA *[]int, instructions *[]string) {
...
func moveToTop1(stackA *[]int, instructions *[]string, count, pos int) {
	*instructions = append(*instructions, "ra")
